...
const AddParcel = () => {
  ...
  const [thanaOfMarchentDistrict, setThanaOfMarchentDistrict] = useState<string[]>([]);
  ...
  //fetch thana of dstrict
  useEffect(() => {
    const fetchThanaOfDistrict = async () => {
      try {
        const response = await getApi(`/apis/district?district=${district}`);
        setThanaOfDistrict(response.data);
      } catch (error) {
        console.log(error.response.data.error);
        toast.error(`${error.response.data.error}`);
      }
    ...
    fetchThanaOfDistrict();
  }, [district]);
  //fetch thana of marchentDstrict
  useEffect(() => {
    const fetchThanaOfMarchentDistrict = async () => {
      try {
        const response = await getApi(`/apis/district?district=${merchantDistrict}`);
        setThanaOfMarchentDistrict(response.data);
      } catch (error) {
        console.log(error.response.data.error);
        toast.error(`${error.response.data.error}`);
      }
    ...
    fetchThanaOfMarchentDistrict();
  }, [merchantDistrict]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isChargeConfirmed) {
            ...
            <form onSubmit={handleSubmit}>
              <div className="p-6.5">
                <div className="flex gap-4 my-5">
                  ...
                  <div
                    onClick={() => setCategory("pick&drop")}
                    className={`${
                      category === "pick&drop" ? "bg-primary" : "bg-[#D9D9D9]"
                    } hover:bg-yellow-100 w-[120px] rounded-md p-3 cursor-pointer`}
                  >
                    <div className="w-8 h-8 bg-black rounded-full flex justify-center items-center mx-auto ">
                      {/* svg 3 */}
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
                      <div className="w-full xl:w-1/2">
                        ...
                        <SelectForDistrict
                          ...
                          onSelect={setMerchantThana}
                          options={thanaOfMarchentDistrict}
                        />
                      </div>
                ...
                <button
                  type="submit"
                  className="flex w-full mt-5 justify-center rounded bg-primary p-3 font-medium text-black hover:bg-opacity-90"
                >
                  Submit
                </button>
              </div>
...
>
  <rect width="20" height="20" fill="url(#pattern0_4844_229)" />
  <defs>
    <pattern
      ...
      height="1"
    >
      <use xlinkHref="#image0_4844_229" transform="scale(0.0111111)" />
    </pattern>
    <image
      id="image0_4844_229"
      width="90"
...
>
  <rect width="20" height="20" fill="url(#pattern0_4844_237)" />
  <defs>
    <pattern
      ...
      height="1"
    >
      <use xlinkHref="#image0_4844_237" transform="scale(0.0111111)" />
    </pattern>
    <image
      id="image0_4844_237"
      width="90"
...

{
  ...
  xmlnsXlink="http://www.w3.org/1999/xlink"
>
  <rect width="20" height="20" fill="url(#pattern0_4844_246)" />
  <defs>
    <pattern
      ...
      height="1"
    >
      <use xlinkHref="#image0_4844_246" transform="scale(0.0111111)" />
    </pattern>
    <image
      id="image0_4844_246"
      width="90"
